...
internal static class BrisbaneTimeConverter
{
    private const string BrisbaneWindowsTimeZone = "E. Australia Standard Time";
    private static readonly TimeSpan BrisbaneOffset = TimeSpan.FromHours(10);

    private static readonly TimeZoneSpec Brisbane = new("Brisbane", BrisbaneWindowsTimeZone, BrisbaneOffset);

    private static readonly TimeZoneSpec[] SourceTimeZones =
    [
        ...
        new("HAST", null, TimeSpan.FromHours(-10)),
        new("HADT", null, TimeSpan.FromHours(-9)),
        ...
        new("BRST", null, TimeSpan.FromHours(-2)),
    ...
    {
        if (!TryParse(input, nowUtc ?? DateTimeOffset.UtcNow, out var parsed, out var error))
        {
            return [ConversionResult.Error(error)];
        }

        var sourceCandidates = BuildSourceCandidates(parsed);
        var results = new List<ConversionResult>();

        foreach (var candidate in sourceCandidates)
        {
            var source = CreateSourceDateTime(candidate.LocalTime, parsed.SourceTimeZone);
            var brisbane = ConvertToBrisbane(source);

            results.Add(new ConversionResult(
                Success: true,
                Title: $"{FormatTime(brisbane)} Brisbane time",
                Subtitle: $"{FormatSource(candidate.LocalTime, parsed.SourceTimeZone, candidate.Assumption)} -> {FormatFull(brisbane)}",
                ClipboardText: FormatClipboard(brisbane),
    ...
    private static bool TryParse(string input, DateTimeOffset nowUtc, out ParsedInput parsed, out string error)
    {
        parsed = default!;
        error = string.Empty;
        ...
        {
            error = "Try a time like 10:30 PM ET, 22:30 EST, or ten thirty CST.";
            return false;
        }

        var working = Normalize(input);
        if (!TryExtractTimeZone(working, out var sourceTimeZone, out working))
        {
            error = "Add a source timezone, for example ET, CET, CEST, UTC, GMT+2, AEST.";
            return false;
        }

        var sourceToday = GetTodayInSourceZone(nowUtc, sourceTimeZone);
        var date = ExtractDate(ref working, sourceToday);

        if (!TryExtractTime(working, out var clock, out var remaining))
        {
            error = "I could not read the time. Try 10:30 PM ET, 22:30 EST, or ten thirty CST.";
            return false;
        }

        date = ParseDate(remaining, date);
        parsed = new ParsedInput(date, clock, sourceTimeZone);
        return true;
    }

    private static IEnumerable<SourceCandidate> BuildSourceCandidates(ParsedInput parsed)
    {
        if (parsed.Clock.IsAmbiguousTwelveHour)
        {
            yield return new SourceCandidate(parsed.Date.ToDateTime(new TimeOnly(parsed.Clock.Hour, parsed.Clock.Minute)), "assuming AM");
            yield return new SourceCandidate(parsed.Date.ToDateTime(new TimeOnly(parsed.Clock.Hour + 12, parsed.Clock.Minute)), "assuming PM");
            yield break;
        }

        yield return new SourceCandidate(parsed.Date.ToDateTime(new TimeOnly(parsed.Clock.Hour, parsed.Clock.Minute)), null);
    }

    private static DateTimeOffset CreateSourceDateTime(DateTime localTime, TimeZoneSpec sourceTimeZone)
    {
        if (sourceTimeZone.FixedOffset is not null)
        {
            return new DateTimeOffset(localTime, sourceTimeZone.FixedOffset.Value);
        }

        var zone = FindTimeZone(sourceTimeZone.WindowsId!, sourceTimeZone.FixedOffset ?? TimeSpan.Zero);
        return new DateTimeOffset(localTime, zone.GetUtcOffset(localTime));
    }

    private static DateTimeOffset ConvertToBrisbane(DateTimeOffset source)
    {
        try
        {
            return TimeZoneInfo.ConvertTime(source, FindTimeZone(Brisbane.WindowsId!, BrisbaneOffset));
        }
        catch (TimeZoneNotFoundException)
        {
            return source.ToOffset(BrisbaneOffset);
        }
    ...
    private static DateOnly GetTodayInSourceZone(DateTimeOffset nowUtc, TimeZoneSpec sourceTimeZone)
    {
        DateTimeOffset sourceNow;
        if (sourceTimeZone.FixedOffset is not null)
        {
            sourceNow = nowUtc.ToOffset(sourceTimeZone.FixedOffset.Value);
        }
        else
        {
            sourceNow = TimeZoneInfo.ConvertTime(nowUtc, FindTimeZone(sourceTimeZone.WindowsId!, TimeSpan.Zero));
        }

        return DateOnly.FromDateTime(sourceNow.DateTime);
    }
    ...
    private static bool TryExtractTimeZone(string input, out TimeZoneSpec sourceTimeZone, out string withoutTimeZone)
    {
        if (TryExtractOffsetTimeZone(input, out sourceTimeZone, out withoutTimeZone))
        {
            return true;
        }

        foreach (var zone in SourceTimeZones.OrderByDescending(zone => zone.Name.Length))
        {
            var pattern = $@"\b{Regex.Escape(zone.Name)}\b$";
            if (Regex.IsMatch(input, pattern, RegexOptions.IgnoreCase))
            {
                sourceTimeZone = zone;
                withoutTimeZone = Regex.Replace(input, pattern, string.Empty, RegexOptions.IgnoreCase).Trim();
                return true;
            }
        }

        sourceTimeZone = default!;
        withoutTimeZone = input;
        return false;
    }

    private static bool TryExtractOffsetTimeZone(string input, out TimeZoneSpec sourceTimeZone, out string withoutTimeZone)
    {
        var match = Regex.Match(input, @"\b(?<prefix>UTC|GMT)\s*(?<sign>[+-])\s*(?<hours>\d{1,2})(?::?(?<minutes>\d{2}))?\b$", RegexOptions.IgnoreCase);
        if (!match.Success)
        {
            sourceTimeZone = default!;
            withoutTimeZone = input;
        ...
        var normalizedName = $"{match.Groups["prefix"].Value.ToUpperInvariant()}{match.Groups["sign"].Value}{hours:00}:{minutes:00}";
        sourceTimeZone = new TimeZoneSpec(normalizedName, null, offset);
        withoutTimeZone = RemoveMatch(input, match);
        return true;
    }
    ...
    private static bool TryExtractRelativeDay(ref string input, DateOnly fallback, out DateOnly date)
    {
        var relativeDays = 0;
        var found = false;
        var replacements = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase)
        {
            ["today"] = 0,
            ["tomorrow"] = 1,
            ["tmr"] = 1,
        ...
        foreach (var (word, days) in replacements)
        {
        ...
        date = fallback.AddDays(relativeDays);
        return found;
    }
    ...
    private static bool TryParseDateText(string text, DateOnly fallback, out DateOnly date)
    {
        ...
        if (!DateTime.TryParse(candidate, culture, DateTimeStyles.AllowWhiteSpaces, out var parsedDate)
    ...
    {
        if (TryExtractNumericTime(input, out clock, out remaining))
        {
            return true;
        }

        if (TryExtractWordTime(input, out clock, out remaining))
        {
            return true;
        }
    ...
    private static bool TryExtractWordTime(string input, out ClockTime clock, out string remaining)
    {
        ...
        var numberValues = words
            .Select((word, index) => new { Word = word, Index = index, Value = TryParseNumberWord(word, out var value) ? value : (int?)null })
    ...
    private static bool TryParseRelativeWordTime(List<string> words, string meridiem, out ClockTime clock, out string remaining)
    {
        ...
        if (pastIndex > 0 && pastIndex + 1 < words.Count && TryParseMinutePhrase(words.Take(pastIndex), out var pastMinutes) && TryParseNumberWord(words[pastIndex + 1], out var pastHour))
        {
            return FinishRelative(pastHour, pastMinutes, meridiem, words, [.. Enumerable.Range(0, pastIndex + 2)], out clock, out remaining);
        }

        if (toIndex > 0 && toIndex + 1 < words.Count && TryParseMinutePhrase(words.Take(toIndex), out var toMinutes) && TryParseNumberWord(words[toIndex + 1], out var toHour))
        {
    ...
    private static bool TryParseMinutePhrase(IEnumerable<string> words, out int minutes)
    {
        ...
        minutes = list.Sum(word => TryParseNumberWord(word, out var value) ? value : 0);
    ...
    private static bool TryParseNumberWord(string word, out int value)
    {
        return NumberWords.TryGetValue(word, out value);
    }
    ...
    private static string FormatTime(DateTimeOffset brisbane)
    {
        return $"{brisbane:h:mm tt}";
    }

    private static string FormatFull(DateTimeOffset brisbane)
    {
        return $"{brisbane:ddd d MMM yyyy, h:mm tt} AEST";
    }

    private static string FormatClipboard(DateTimeOffset brisbane)
    {
        return $"{brisbane:yyyy-MM-dd HH:mm} AEST";
    }

    private sealed record TimeZoneSpec(string Name, string? WindowsId, TimeSpan? FixedOffset);

    private sealed record ParsedInput(DateOnly Date, ClockTime Clock, TimeZoneSpec SourceTimeZone);
